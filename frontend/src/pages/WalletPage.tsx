...
export const WalletPage: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <div className="bg-white/50 backdrop-blur-xl border border-white/40 rounded-3xl p-8 lg:p-10 shadow-sm shadow-slate-900/5 relative overflow-hidden transition-all">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-8 relative z-10">
          ...
          {/* Address Box */}
          <div className="bg-white/45 border border-slate-300/50 shadow-inner rounded-2xl p-5 lg:min-w-[440px]">
            ...
            <div className="flex items-center justify-between gap-3 mt-2 bg-white/70 px-4 py-3 rounded-xl border border-slate-300/50 shadow-sm transition-all hover:border-indigo-200">
              ...
              <button
                onClick={copyAddress}
                className="text-slate-500 hover:text-indigo-700 bg-white/60 hover:bg-indigo-50 p-2 rounded-lg transition-colors flex items-center gap-1 border border-transparent hover:border-indigo-100"
                title="Copy Address"
              >
                {copied ? (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white/45 backdrop-blur-xl border border-white/40 shadow-sm shadow-slate-900/5 rounded-2xl p-6 lg:p-8 hover:bg-white/65 hover:shadow-md hover:-translate-y-1 transition-all duration-300">
        ...
        <div className="bg-white/45 backdrop-blur-xl border border-white/40 shadow-sm shadow-slate-900/5 rounded-2xl p-6 lg:p-8 hover:bg-white/65 hover:shadow-md hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="text-3xl font-bold font-mono text-sky-700">
            ${analytics?.daily_spend.toFixed(4) || '0.0000'} <span className="text-xl text-slate-500">/ ${policy?.daily_limit.toFixed(2) || '2.00'}</span>
          </div>
          <p className="text-xs font-medium text-slate-600 mt-2">
        ...
        <div className="bg-white/45 backdrop-blur-xl border border-white/40 shadow-sm shadow-slate-900/5 rounded-2xl p-6 lg:p-8 hover:bg-white/65 hover:shadow-md hover:-translate-y-1 transition-all duration-300">
          ...
          <div className="text-3xl font-bold font-mono text-emerald-700">
            ${analytics?.monthly_spend.toFixed(4) || '0.0000'} <span className="text-xl text-slate-500">/ ${policy?.monthly_limit.toFixed(2) || '20.00'}</span>
          </div>
          <p className="text-xs font-medium text-slate-600 mt-2">
