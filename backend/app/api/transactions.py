...
from app.database.models import Service, Transaction, Wallet
...
async def get_analytics_summary(db: AsyncSession = Depends(get_db)):
    ...
    # Counts
    res_all_tx = await db.execute(select(func.count(Transaction.id)))
    ...
    res_fail_tx = await db.execute(
        select(func.count(Transaction.id)).where(Transaction.status.in_(["rejected", "failed"]))
    )
    ...
    # Wallet balance
    res_wallet = await db.execute(select(Wallet).limit(1))
    wallet = res_wallet.scalars().first()
    wallet_balance = wallet.balance if wallet else 10.00
